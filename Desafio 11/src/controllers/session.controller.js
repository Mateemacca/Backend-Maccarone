...
export const loginUser = (req, res) => {
    passport.authenticate('login', { failureRedirect: '/faillogin' })(req, res, async () => {
        try {
            const user = await userRepository.loginUser(req.body.email, req.body.password);
            req.session.user = {
                first_name: user.first_name,
                last_name: user.last_name,
                age: user.age,
...
export const restorePassword = async (req, res) => {
    ...
    try {
        ...
        res.send('<p>Contraseña actualizada correctamente</p> <a href="/login">volver a iniciar sesion</a>');
    } catch (error) {
        console.error(error);
